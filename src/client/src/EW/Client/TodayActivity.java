...
public class TodayActivity extends ListActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
    	...
    	String dfim = Year + "-" + (Month + 1) + "-" + Day;
        
		// get day
    	Main.todayevents=SearchEvents(dinicio, dfim);
    	
        if (Main.todayevents==null) {
        	return;
        }
        int size=Main.todayevents.length;
        if (size==0) {
        	return;
        }
        ...
        for (int i=0;i<size;i++) {
        	names[i] = Main.todayevents[i].Name;
        	places[i] = Main.todayevents[i].Place;
        	if (Main.checkedevents.contains(Main.todayevents[i].Ide)) {
        		images[i] = R.drawable.ok;
        		Main.todayevents[i].Check=true;
        	} else {
        		images[i] = 0;
        		Main.todayevents[i].Check=false;
        	}
        }
		...
		//Toast.makeText(getApplicationContext(), getApplicationContext().toString(), Toast.LENGTH_SHORT).show();
    	...
    	lv.setOnItemClickListener(new OnItemClickListener() {
    		...
    		public void onItemClick(AdapterView<?> parent, View view,
    			...
    			clicked = position;
    			String ide = Main.todayevents[position].Ide;
    			boolean check = Main.todayevents[position].Check;
    			
    			Main.todayevents[position] = getEventDetail(ide);
    			Main.todayevents[position].Check=check;
    			
    			EventActivity.evt=Main.todayevents[position];
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    	...
    	Toast.makeText(getApplicationContext(), "finalmente", Toast.LENGTH_SHORT).show();
    	if (requestCode==0)
    	{
    		switch (resultCode) { 
	    		case 1:
	    			...
	    			Main.todayevents[clicked].Check=true;
	    			if (!Main.checkedevents.contains(Main.todayevents[clicked].Ide)) {
	    				Main.checkedevents.add(Main.todayevents[clicked].Ide);
	    			}
	    			this.setListAdapter(new MyArrayAdapter(this, names, places, images));
	    			mark(Main.todayevents[clicked].Ide, "check");
	    			putCalendar(Main.todayevents[clicked]);
	    			try {
	    		...
	    		case 2:
	    			...
	    			images[clicked]=0;
	    			Main.todayevents[clicked].Check=false;
	    			if (Main.checkedevents.contains(Main.todayevents[clicked].Ide)) {
	    				Main.checkedevents.remove(Main.todayevents[clicked].Ide);
	    			}
	    			this.setListAdapter(new MyArrayAdapter(this, names, places, images));
	    			mark(Main.todayevents[clicked].Ide, "uncheck");
	    			rmCalendar(Main.todayevents[clicked]);
	    			try {
	    				FileOutputStream fos = openFileOutput("EWCche", Context.MODE_PRIVATE);
	    				for (int i=0;i<Main.checkedevents.size();i++) {
	    					String temp = Main.checkedevents.get(i) + "\n";
	...
	private Event[] SearchEvents(String dinicio, String dfim) {
        ...
        dialog.setCancelable(true);
        // set a message text
        dialog.setMessage("Fetching Today's Events...");
        // show it
        dialog.show();
        
        //String url = Main.IP + "/events?oper=searchevent&dinicio=" + dinicio + "&dfim=" + dfim;
        String url = Main.IP + "/events?oper=searchevent&dfim=" + dfim;
        ...
        Toast.makeText(getApplicationContext(), "Today's Events loaded", Toast.LENGTH_SHORT).show();
        dialog.dismiss();
		return revents;
	}
