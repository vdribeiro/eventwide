...
public class WeekActivity extends ListActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		// get week
    	Main.weekevents=SearchEvents(dinicio, dfim);
    	
        if (Main.weekevents==null) {
        	return;
        }
        int size=Main.weekevents.length;
        if (size==0) {
        	return;
        }
        ...
        for (int i=0;i<size;i++) {
        	names[i] = Main.weekevents[i].Name;
        	places[i] = Main.weekevents[i].Place;
        	if (Main.checkedevents.contains(Main.weekevents[i].Ide)) {
        		images[i] = R.drawable.ok;
        		Main.weekevents[i].Check=true;
        	} else {
        		images[i] = 0;
        		Main.weekevents[i].Check=false;
        	}
        }
    	...
    	lv.setOnItemClickListener(new OnItemClickListener() {
    		...
    		public void onItemClick(AdapterView<?> parent, View view,
    			...
    			clicked = position;
    			String ide = Main.weekevents[position].Ide;
    			boolean check = Main.weekevents[position].Check;
    			
    			Main.weekevents[position] = getEventDetail(ide);
    			Main.weekevents[position].Check=check;
    			
    			EventActivity.evt=Main.weekevents[position];
    	...
    	if (requestCode==0)
    	{
    		switch (resultCode) { 
	    		case 1:
	    			...
	    			Main.weekevents[clicked].Check=true;
	    			if (!Main.checkedevents.contains(Main.weekevents[clicked].Ide)) {
	    				Main.checkedevents.add(Main.weekevents[clicked].Ide);
	    			}
	    			this.setListAdapter(new MyArrayAdapter(this, names, places, images));
	    			mark(Main.weekevents[clicked].Ide,"check");
	    			putCalendar(Main.weekevents[clicked]);
	    			try {
	    		...
	    		case 2:
	    			...
	    			images[clicked]=0;
	    			Main.weekevents[clicked].Check=false;
	    			if (Main.checkedevents.contains(Main.weekevents[clicked].Ide)) {
	    				Main.checkedevents.remove(Main.weekevents[clicked].Ide);
	    			}
	    			this.setListAdapter(new MyArrayAdapter(this, names, places, images));
	    			mark(Main.weekevents[clicked].Ide,"uncheck");
	    			rmCalendar(Main.weekevents[clicked]);
	    			try {
	    				FileOutputStream fos = openFileOutput("EWCche", Context.MODE_PRIVATE);
	    				for (int i=0;i<Main.checkedevents.size();i++) {
	    					String temp = Main.checkedevents.get(i) + "\n";
    ...
    private Event[] SearchEvents(String dinicio, String dfim) {
        ...
        dialog.setCancelable(true);
        // set a message text
        dialog.setMessage("Fetching Week's Events...");
        // show it
        dialog.show();
        ...
        Toast.makeText(getApplicationContext(), "Week's Events loaded", Toast.LENGTH_SHORT).show();
        dialog.dismiss();
		return revents;
	}
